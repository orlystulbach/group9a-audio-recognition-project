#!/usr/bin/env python3
"""
Train Voice-Adaptive Model with Existing Voice Samples
"""
...
EXISTING_VOICE_SAMPLES_DIR = 'my_voice_samples'
EPOCHS = 100
BATCH_SIZE = 32
LEARNING_RATE = 0.001
...
def load_combined_dataset():
    """
    Load and combine original dataset with existing voice samples
    """
    ...
    if os.path.exists(EXISTING_VOICE_SAMPLES_DIR):
        print(f"Loading existing voice samples from {EXISTING_VOICE_SAMPLES_DIR}...")
        
        for digit in range(10):
            digit_dir = os.path.join(EXISTING_VOICE_SAMPLES_DIR, str(digit))
            if os.path.exists(digit_dir):
                wav_files = [f for f in os.listdir(digit_dir) if f.endswith('.wav')]
                print(f"Found {len(wav_files)} samples for digit {digit}")
        ...
        print(f"❌ Voice samples directory {EXISTING_VOICE_SAMPLES_DIR} not found!")
        return None, None
...
def train_with_existing_voice_samples():
    """
    Train the voice-adaptive model using existing voice samples
    """
    print("\n🤖 Training Voice-Adaptive Model with Existing Voice Samples")
    print("=" * 70)
    
    # Check if voice samples exist
    if not os.path.exists(EXISTING_VOICE_SAMPLES_DIR):
        print(f"❌ Voice samples directory {EXISTING_VOICE_SAMPLES_DIR} not found!")
        print("Please make sure your voice samples are in the my_voice_samples directory.")
    ...
    if X is None or len(X) == 0:
        print("❌ No data found! Please check your voice samples directory.")
        return None, None, None
...
if __name__ == "__main__":
    train_with_existing_voice_samples() 