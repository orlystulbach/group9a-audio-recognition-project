...
class SpeechInputClassifier:
    def __init__(self, model_path='fixed_digit_classifier_model.h5', sample_rate=22050, duration=2):
        """
        Initialize the speech input classifier
        ...
        # Initialize scaler (you might need to fit this with your training data)
    ...
    def interactive_mode(self):
        """
        Run interactive mode for continuous digit recognition
        """
        if self.model is None:
            print("❌ Model not loaded. Cannot start interactive mode.")
            return
        
        print("\n🎤 Interactive Digit Recognition Mode")
        print("=" * 40)
        print("Commands:")
        ...
        while True:
            try:
                user_input = input("\nPress ENTER to record (or type 'quit'/'save'): ").strip().lower()
...
def main():
    """
    Main function to run the speech input classifier
    """
    print("🎤 Speech Input Digit Classifier")
    print("=" * 40)
    ...
    if not available_models:
        ...
        print("  - fixed_train_digit_classifier.py")
        print("  - deeper_train_digit_classifier.py")
        print("  - improved_train_digit_classifier.py")
        return
